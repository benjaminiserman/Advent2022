...
internal static class Day24_1
{
	...
	public static string Execute()
	{
		...
		var queue = new PriorityQueue<((int x, int y), BoardState state, List<(int x, int y)> positions), int>();
		var closedSet = new HashSet<((int x, int y), int t)>();

		(int x, int y) start = openTiles.Where(t => t.y == yMin).MinBy(t => t.x);
		(int x, int y) target = openTiles.Where(t => t.y == yMax).MaxBy(t => t.x);

		int Heuristic((int x, int y) pos, BoardState state)
		{
			//return state.Time;
		...
		result = int.MaxValue;
		queue.Enqueue((start, boardStates.First(), new() { start }), Heuristic(start, boardStates.First()));
		int maxTime = 0;
		long a = 0;

		while (queue.TryDequeue(out var t, out var p))
		{
			var ((x, y), state, list) = t;

			var currentState = GetBoardState(state.Time + 1);
			//Console.WriteLine($"{currentState.Time}: {x}, {y}");

			//if (currentState.Time + p - 1000 >= result)
			//{
			//	continue;
			//}

			//if (state.Time % 100 == 0 && state.Time > maxTime)
			//{
			//	maxTime = state.Time;
			...
			//	Console.WriteLine($"t: {queue.Peek().state.Time}, p: {Heuristic(queue.Peek().Item1, queue.Peek().state)}");
			//}
			...
			if (x == target.x && y == target.y)
			{
				...
				//foreach (var boardState in boardStates.Skip(1))
				//{
				//	boardState.Print(xMin, xMax, yMin, yMax);
				//}
			...
			bool Try(int x, int y)
			{
				if (openTiles.Contains((x, y)) 
					...
					&& !closedSet.Contains(((x, y), currentState.Time)))
				{
					closedSet.Add(((x, y), currentState.Time));
					queue.Enqueue(((x, y), currentState, null/*new(list)
					{
						(x, y)
					}*/), Heuristic((x, y), currentState));
					return true;
				}
